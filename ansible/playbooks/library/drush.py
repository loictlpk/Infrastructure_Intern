...
DOCUMENTATION = ''' https://github.com/berenddeboer/ansible-drush-module/blob/master/drush.py
---
module: drush
...
options:
    path:
        description:
            - Root of your Drupal site. (Path of drupal project)
        required: true
    command:
        description:
...
# SECTION MODIFIÉE : le format JSON provoque une erreur.
# #####################################################################################################
    # LIGNE ORIGINALE :
    # cmd = "%s %s --nocolor --strict=0 --format=json -y %s" % (drush, command, cmd_extra)
    #  LIGNE MODIFIÉE
    cmd = "%s %s --nocolor --strict=0 --yes %s" % (drush, command, cmd_extra)
#####################################################################################################
